...
> **Основной сценарий проекта** — запуск на **своём компьютере**: `git clone` → `.env` → `uvicorn` → http://127.0.0.1:8765.  
> Этот документ — для тех, кто сознательно хочет Docker, systemd или доступ по сети. Для обычных пользователей репозитория он **не нужен**.
...
| Сценарий | Host | Auth |
|----------|------|------|
| **Типичное использование (clone & run)** | `127.0.0.1` | Не требуется |
